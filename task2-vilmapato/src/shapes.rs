...
impl Rectangle {
    pub fn new(width: f64, height: f64) -> Result<Self, Error> { //this defines a new rectangle instance
        if width < 0.0 {
    ...
    pub fn set_width(&mut self, width: f64) -> Result<(), Error> { //this sets the width of the rectangle but checks for validity
        if width < 0.0 {
            return Err(Error::InvalidWidth);
        }
        self.width = width;
